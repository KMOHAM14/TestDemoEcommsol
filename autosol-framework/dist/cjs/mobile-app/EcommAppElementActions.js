...
class EcommAppElementActions {
     ...
     * @param waitTimeOut - The timeout for element display (default: ETimeout.TIMEOUT_S).
     */
    static async longPressOnElement(elementPath, timeDuration = constants_1.ETimeout.TIMEOUT_S, waitTimeOut = constants_1.ETimeout.TIMEOUT_S) {
        await elementPath.waitForDisplayed({ timeout: waitTimeOut });
     ...
     * @param waitTimeOut - The timeout for element display (default: ETimeout.TIMEOUT_S).
     */
    static async getTextAndCompareTextStartsWith(elementPath, expectedText, waitTimeOut = constants_1.ETimeout.TIMEOUT_S) {
        const string = await this.getText(await elementPath, waitTimeOut);
        assert_1.default.equal(await string.startsWith(expectedText), true);
    }
    /**
     ...
     * @param waitTimeOut - The timeout for element display (default: ETimeout.TIMEOUT_S).
     */
    static async getTextAndCompareTextEndsWith(elementPath, expectedText, waitTimeOut = constants_1.ETimeout.TIMEOUT_S) {
        const string = await this.getText(await elementPath, waitTimeOut);
        // const assert = require('assert');
        assert_1.default.equal(await string.endsWith(expectedText), true);
    }
    /**
    ...
    static async waitForIsShown(isShown = true) {
        const selector = driver.isIOS
            ? exports.SELECTORS.IOS_PICKERWHEEL
            : exports.SELECTORS.ANDROID_LISTVIEW;
    ...
    static async setAndroidValue(value) {
        await $(`${exports.SELECTORS.ANDROID_LISTVIEW}/*[@text='${value}']`).click();
    }
    /**
    ...
    static async setIOSValue(value) {
        await $(exports.SELECTORS.IOS_PICKERWHEEL).addValue(value);
        await $(exports.SELECTORS.DONE).click();
    }
}
exports.default = EcommAppElementActions;
