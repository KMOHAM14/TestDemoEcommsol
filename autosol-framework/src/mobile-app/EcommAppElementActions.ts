...
class EcommAppElementActions {
   ...
   * @param waitTimeOut - The timeout for element display (default: ETimeout.TIMEOUT_S).
   */
  static async longPressOnElement(
    elementPath: WebdriverIO.CucumberOpts,
    timeDuration = ETimeout.TIMEOUT_S,
    waitTimeOut = ETimeout.TIMEOUT_S,
  ) {
    await elementPath.waitForDisplayed({ timeout: waitTimeOut });
    ...
    await driver.touchAction([
      { action: 'press', x, y },
      { action: 'wait', ms: timeDuration }, // Adjust duration as needed
      { action: 'release' },
    ]);
  }
   ...
   * @param waitTimeOut - The timeout for element display (default: ETimeout.TIMEOUT_S).
   */
  static async getTextAndCompareTextStartsWith(
    elementPath: WebdriverIO.CucumberOpts,
    expectedText: string,
    waitTimeOut = ETimeout.TIMEOUT_S,
  ) {
   ...
   * @param waitTimeOut - The timeout for element display (default: ETimeout.TIMEOUT_S).
   */
  static async getTextAndCompareTextEndsWith(
    elementPath: WebdriverIO.CucumberOpts,
    expectedText: string,
    waitTimeOut = ETimeout.TIMEOUT_S,
  ) {
   ...
   */
  static async verifyGreaterThan(SmallNumber: string, LargeNumber: string) {
   ...
   * @param LargeNumber - The timeout for element display (default: ETimeout.TIMEOUT_S).
   */
  static async verifyLessThan(SmallNumber: string, LargeNumber: string) {
    const smallNum = parseInt(SmallNumber, 10);
    const largeNum = parseInt(LargeNumber, 10);
   ...
   * @param LargeNumber - The timeout for element display (default: ETimeout.TIMEOUT_S).
   */
  static async verifyLessThanOrEqual(SmallNumber: string, LargeNumber: string) {
    const smallNum = parseInt(SmallNumber, 10);
    const largeNum = parseInt(LargeNumber, 10);
   ...
   */
  static async verifyElementIsDisplayed(elementPath: WebdriverIO.Element) {
   ...
   * @param elementPath - The element to verify.
   */
  static async verifyElementIsNotDisplayed(elementPath: WebdriverIO.Element) {
...
/**
   * Generate a random email with random String, random Numbers and email starts with user.
   * @returns The generated random email address.
   */
    static getRandomEmail(): string[] {
      const randomString = Math.random().toString(36).substring(2, 8);
      const randomNumbers = Math.floor(Math.random() * 10000);
      const customerEmailWithoutDonaminName = `user${randomNumbers}${randomString}`
      const customerEmail = `user${randomNumbers}${randomString}@yopmail.com`;
      console.log(`customerEmail-->, ${customerEmail}`);
      console.log(`customerEmailWithoutDomainName -->, ${customerEmailWithoutDonaminName}`);
      return [ customerEmailWithoutDonaminName, customerEmail ];
    }
   ...
   */
  static async setAndroidValue(value: string) {
   ...
   * @param value - The value to set.
   */
  static async setIOSValue(value: string) {
